...
Note that end users, who just want to perform robust
differential expression analysis using \Rpackage{TCC},
can skip this chapter (3 Normalization) and move from here to,
for example, the next chapter (4 Differential expression).
...
in the next section (\ref{section-4} Differential expression).
...
\label{norm_twogroup_without_reps}
...
group <- data.frame(group = group, pair = pair)
...
size.factors <- tcc$norm.factors * colSums(tcc$count)
...
norm.factors <- norm.factors / mean(norm.factors)
size.factors <- norm.factors * colSums(tcc$count)
size.factors <- size.factors / mean(size.factors)
sizeFactors(cds) <- size.factors
cds <- estimateDispersions(cds, method="blind", sharingMode="fit-only")
result <- nbinomTest(cds, 1, 2)
