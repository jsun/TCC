...
Let us begin by showing two examples (Cases 1 and 2) of identifying DEGs 
...
\textbf{Case 1}:
DE analysis of two-group count data with replicates by 
using the exact test (Robinson and Smyth, 2008) 
...
recommended this analysis pipeline. 
...
is recommended (Hardcastle and Kelly, 2010). 
Suggested citations are as follows: \Rpackage{TCC} (Sun et al., 2013),
TbT (Kadota et al., 2012), 
TMM (Robinson and Oshlack, 2010), 
\Rpackage{baySeq} (Hardcastle and Kelly, 2010), 
...
iDEGES/DESeq normalization 
(i.e., the iDEGES/DESeq-DESeq combination). 
...
and \Rpackage{DESeq} (Anders and Huber, 2010). For details, 
...
consisting of functions in other packages (\Rpackage{edgeR}, \Rpackage{DESeq},
...
\begin{table}[h]
  ...
  \caption{\ref{ss_norm_multifactor_reps} unpaired samples (multi-group)}
    \begin{tabular}{|ccc|ccc|}
      \hline
      Label & Exmaple 1 & Example 2 & Label & Exmaple 3 & Example 4 \\ \hline
...
case above, this DESeq-DESeq-DESeq 
pipeline (DEGES/DESeq) changes the corresponding arguments 
...
functions in \Rpackage{DESeq}, to perform the DEGES/DESeq 
pipeline without \Rpackage{TCC}. The \Rfunction{calcNormFactors} function 
...
(including \Rpackage{edgeR}, \Rpackage{DESeq}, and \Rpackage{baySeq})
...
We here describe two DEGES-based normalization pipelines
(\ref{ss_2gs_degesedger} DEGES/edgeR and
\ref{ss_2gs_degesdeseq} DEGES/DESeq) using the subset of \Robject{hypoData}
...
d <- calcNormFactors(d)
norm.factors <- d$samples$norm.factors * colSums(hypoData[!is.DEG, c(1, 4)]) / colSums(hypoData[, c(1, 4)])
norm.factors <- norm.factors / mean(norm.factors)
norm.factors
@

\subsubsection{DEGES/DESeq}
\label{ss_2gs_degesdeseq}

The DEGES/DESeq pipeline for two-group data without
replicates can be performed as follows:
...
For \Rpackage{DESeq} users, we provide commands,
consisting of functions in \Rpackage{DESeq},
to perform the DEGES/DESeq pipeline without \Rpackage{TCC}.
The \Robject{calcNormFactors} function together with the above
...
\Rpackage{edgeR} and \Rpackage{DESeq} (and \Rpackage{DESeq2}) employs generalized
...
result <- topTags(lrt, n = nrow(hypoData))
result <- result$table[rownames(hypoData), ]
if (sum(result$FDR < FDR) > (floorPDEG * nrow(hypoData))){
...
\subsubsection{DEGES/DESeq}
\label{nrom_twogroup_without_reps_degesdeseq}

The DEGES/DESeq pipeline for two-group paired data can be performed as follows:
...
parameter settings can be regarded as a wrapper function for the following commands.
...
DEGES/edgeR, and \ref{section-3-3-3} DEGES/DESeq).
...
The DEGES/DESeq pipeline with the two models
(\Robject{fit1} and \Robject{fit0}) can be performed as follows.

<<degesDeseq_multigroup_tcc_nf>>=
tcc <- calcNormFactors(tcc, norm.method = "deseq", test.method = "deseq",
                       iteration = 1, fit1 = fit1, fit0 = fit0)
tcc$norm.factors
@

The two models (\Robject{fit1} and \Robject{fit0}) will automatically be
...
in \Rpackage{DESeq}, to perform the DEGES/DESeq pipeline without 
\Rpackage{TCC}. The \Rfunction{calcNormFactors} function together with the 
above parameter settings can be regarded as a wrapper function for the 
following commands.

<<degesDeseq_multigroup_deseq>>=
library(TCC)
data(hypoData_mg)
group <- c(1, 1, 1, 2, 2, 2, 3, 3, 3)
FDR <- 0.1
...
\Rfunction{estimateSizeFactors} function in both \Rpackage{DESeq} and \Rpackage{DESeq2}. Note that the 
...
The size factors in both \Rpackage{DESeq} and \Rpackage{DESeq2} package are comparable to 
...
Similar to the DEGES/edgeR case,
the DEGES/DESeq-normalized data can be retrieved as follows.

<<degesDeseq_tcc_normed_data>>=
library(TCC)
data(hypoData)  
group <- c(1, 1, 1, 2, 2, 2)
nonDEG <- 201:1000
...
For comparison, the summary statistics for DESeq-normalized
data produced using the original normalization method in 
\Rpackage{DESeq} can be obtained as follows.
...
range(apply(normalized.count[nonDEG, ], 2, median))
@
<<1330110292,echo = false, result = hide>>=
buff_2 <- range(apply(normalized.count[nonDEG, ], 2, median))
@
...
to
$\Sexpr{sprintf("%.5f", max(buff_1))}$
) from DEGES/DESeq-normalized data are closer to the truth
(i.e., ranging from $\Sexpr{sprintf("%.1f", min(hypoData.median))}$ to 
$\Sexpr{sprintf("%.1f",max(hypoData.median))}$)
than those (ranging from 
$\Sexpr{sprintf("%.5f", min(buff_2))}$
...
the DEGES/DESeq-normalized data without
replicates can be retrieved as follows.

<<degesDeseq_tcc_without_replicates_normed_data>>=
library(TCC)
data(hypoData)  
nonDEG <- 201:1000
group <- c(1, 2)
...
apply(normalized.count[nonDEG, ], 2, median)
@
<<1664110292,echo = false, result = hide>>=
buff_1 <- range(apply(normalized.count[nonDEG, ], 2, median))
@

The same procedure consisting of functions in \Rpackage{DESeq} is 

<<degesDeseq_deseq_without_replicates_normed_data>>=
library(TCC)
data(hypoData) 
nonDEG <- 201:1000
group <- c(1, 2)
...
(i.e., ranging from $\Sexpr{sprintf("%.1f", min(hypoData.14.median))}$ to 
$\Sexpr{sprintf("%.1f",max(hypoData.14.median))}$)
than those (
$\Sexpr{sprintf("%.5f", min(buff_2))}$
and
$\Sexpr{sprintf("%.5f", max(buff_2))}$
...
\subsubsection{Retrieving two-group DEGES/DESeq-normalized data without replicates (paired)}
\label{ss_ret_twogroup_degesdeseq_without_reps}
...
range(apply(normalized.count[nonDEG, ], 2, median))
@
<<5490200292,echo = false, result = hide>>=
buff_1 <- range(apply(normalized.count[nonDEG, ], 2, median))
@
...
For comparison, the summary statistics for DESeq-normalized data
produced using the original normalization method in \Rpackage{DESeq}
...
cl <- data.frame(group = group, pair = pair)
tcc <- new("TCC", hypoData, cl)
tcc <- calcNormFactors(tcc, norm.method = "deseq", iteration = 0, paired = TRUE)
normalized.count <- getNormalizedData(tcc)
apply(normalized.count[nonDEG, ], 2, median)
range(apply(normalized.count[nonDEG, ], 2, median))
@
<<9600320992,echo = false, result = hide>>=
buff_2 <- range(apply(normalized.count[nonDEG, ], 2, median))
@
...
paired two-group data without replicates (\ref{faefljifeaf}), 
...
<<maplot_of_estimated_result, fig = TRUE, size = 0.60, png = TRUE, pdf = FALSE, eps = FALSE>>=
plot(tcc)
@
...
d <- estimateTagwiseDisp(d)
out <- exactTest(d)
result <- topTags(out, n=nrow(hypoData), sort.by="PValue")
head(result)
@
...
We give a procedure for DE analysis using the likelihood ratio test for GLMs
implemented in \Rpackage{DESeq2} together with iDEGES/DESeq2 normalization
factors (i.e., the iDEGES/DESeq2-DESeq2 combination) for the hypothetical
two-group count data with replicates (i.e., the hypoData object).
If the user wants to determine the genes having
an FDR threshold of $< 10$\% as DEGs,
...
The plot function generates an M-A plot, where "M" indicates the
log-ratio 
...
<<3489400103, fig = TRUE, size = 0.60, png = TRUE, pdf = FALSE, eps = FALSE>>=
plot(tcc)
@
...
AUC(rocdemo.sca(truth = truth, data = -rank(res$pvalue)))
@
...
the size factors termed in \Rpackage{DESeq2} and \Rpackage{DESeq}
are comparable to the normalized effective library sizes termed
...
@
<<4090911011, echo = FALSE>>=
buff_2 <- AUC(rocdemo.sca(truth = truth, data = -rank(res$pvalue)))
@

The different AUC values ($\Sexpr{buff_1}$ and $\Sexpr{buff_2}$) caused by the different
choices of test methods: the former and the latter procedures used the
\Rfunction{nbinomLRT} and the \Rfunction{nbinomWaldTest} functions,
respectively. \Rpackage{TCC} internally uses the \Rfunction{nbinomLRT}
function in \Rpackage{DESeq2} for this experimental design.
...
iDEGES/DESeq normalization factors 
(i.e., the iDEGES/DESeq-DESeq combination) 
...
<<idegerDeseq_tcc_without_replicaets_summary>>=
library(TCC)
data(hypoData)
group <- c(1, 2)
tcc <- new("TCC", hypoData[, c(1, 4)], group)
...
We show a procedure for DE analysis using the GLM aproach implemented in
\Rpackage{edgeR} together with iDEGES/edgeR normalization 
factors for paired two-group count data without replicates.
...
DEGES/edgeR normalization.

<<degerTbt_tcc_multigroup_summary>>=
set.seed(1000)
samplesize <- 100
...
<<degerTbt_tcc_edger_org_multigroup_summary>>=
library(TCC)
data(hypoData_mg)
group <- c(1, 1, 1, 2, 2, 2, 3, 3, 3)
design <- model.matrix(~ as.factor(group))
...
DEGES/edgeR normalization can be performed as follows.

<<degerEdger_tcc_edger_org_multigroup_summary_2>>=
library(TCC)
data(hypoData_mg)
group <- c(1, 1, 1, 2, 2, 2, 3, 3, 3)
tcc <- new("TCC", hypoData_mg, group)
...
DEGES/edgeR normalization 
(i.e., the DEGES/edgeR-DESeq combination) 
can be performed as follows.

<<degerEdger_tcc_deseq_tcc_multigroup_summary>>=
library(TCC)
data(hypoData_mg)
group <- c(1, 1, 1, 2, 2, 2, 3, 3, 3)
tcc <- new("TCC", hypoData_mg, group)
...
<<degerEdger_tcc_deseq_org_multigroup_summary>>=
library(TCC)
data(hypoData_mg)
group <- c(1, 1, 1, 2, 2, 2, 3, 3, 3)
tcc <- new("TCC", hypoData_mg, group)
...
ef.libsizes <- tcc$norm.factors * colSums(tcc$count)
size.factors <- ef.libsizes / mean(ef.libsizes)
sizeFactors(cds) <- size.factors
cds <- estimateDispersions(cds)
reduced.model <- fitNbinomGLMs(cds, fit0)
full.model <- fitNbinomGLMs(cds, fit1)
p.value <- nbinomGLMTest(full.model, reduced.model)
...
Now let us see how the DEGES/DESeq-DESeq combination 
with the original DESeq-DESeq combination 
performs. First, we calculate the AUC value for the ranked 
gene list obtained from the DEGES/DESeq-DESeq 
combination.
...
(i.e., the DESeq-DESeq combination).
...
It can be seen that the DEGES/DESeq-DESeq combination 
...
<<maplot_simulate_data_1000, fig = TRUE, size = 0.8, png = TRUE, pdf = FALSE, eps = FALSE>>=
plot(tcc)
@
...
<<maplot_normed_simulate_data_1000, fig = TRUE, size = 0.8, png = TRUE, pdf = FALSE, eps = FALSE>>=
...
<<maplot_normed_simulate_data_1000_2, fig = TRUE, size = 0.8, png = TRUE, pdf = FALSE, eps = FALSE>>=
tcc <- calcNormFactors(tcc, norm.method = "tmm", test.method = "edger", 
                       iteration = 3, FDR = 0.1, floorPDEG = 0.05)
