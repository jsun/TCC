...
\textbf{Case 1}: DE analysis of two-group count data with replicates by 
...
\textbf{Case 2}: DE analysis for two-group count data with replicates by using 
...
\textbf{Case 3}: DE analysis for two-group count data without replicates by using 
...
(2) aligning reads to the reference genome or transcriptome using Rbowtie,
and (3) producing count matrix from aligned reads.
\Rpackage{TCC} use the raw count data as input.
As also clearly mentioned by \Rpackage{DESeq2},
...
CPM (counts per million), and RPKM. The count data must be raw counts of reads.


\subsection{Reading the count data}
\label{reading_the_count_data}
...
can skip this chapter (\ref{s_normalization} Normalization) and move from here to,
for example, the next chapter (\ref{s_differential_expression} Differential expression).
...
\begin{table}[H]
  ...
  \caption{\ref{ss_norm_twogroup_reps} unpaired samples (two-group)}
    \begin{tabular}{|ccc|ccc|}
      \hline
      Label & exmaple 1 & exammple 2 & label & exmaple 3 & example 4 \\ \hline
...
pipeline. That is, the \Rfunction{calcNormFactors} function with the 
...
<<degesDeseq_without_replicates_tcc>>=
tcc <- calcNormFactors(tcc, norm.method = "deseq", test.method = "deseq",
                       iteration = 1, FDR = 0.1, floorPDEG = 0.05)
tcc$norm.factors
@
...
<<degesDeseq_without_replicates_deseq>>=
library(TCC)
data(hypoData)
group <- c(1, 2)
FDR <- 0.1
...
\Rpackage{edgeR} employs generalized linear models (GLMs) which can apply
to detect DEGs from paired two-group count data.
The GLMs approach, packaged in \Rpackage{edgeR}, requires the arguments
for creating experiment design to perform DE analysis.
\Rpackage{TCC} supports such analysis and creates those
paramenters automatically.
User only should create the group information for \Robject{TCC} object,
and just execute the \Rfunction{calcNormFactors} with \Robject{paired = TRUE}
argument.

We here explain the approach for normalizing paired two-group count data.
We treate \Robject{hypoData} as paired two-group data for explaining the
approach here.
...
An advantage of this DEGES/\Rfunction{edgeR} pipeline is that the multi-step
normalization strategy only needs the methods in the \Rpackage{edgeR}.
These factors should be the same as those produced by the following
procedure consisting of functions implemented in \Rpackage{edgeR}.
...
Similer to \Rpackage{edgeR}, \Rpackage{DESeq} also employs GLMs approach
to perform DE analysis for paired two-group count data.
User performs GLMs approach only should execute the
\Rfunction{calcNormFactors} function with \Robject{paired = TRUE} argument.
...
These factors should be same as those produced by the following
procedure consisting of functions implemented in \Rpackage{DESeq}.
...
<<deseq_deseq_without_replicates_normed_data>>=
library(TCC)
data(hypoData)
nonDEG <- 201:1000
group <- c(1, 2)
...
We treate \Robject{hypoData} as the data of paired two-group experiment
for explaining to retrieve paired two-group DEGES/\Rpackage{edgeR}-normalized
data. The normalized data can be retrieved as follows by functions of
\Rpackage{TCC} package.
...
original normalization method (i.e., TMM) in \Rpackage{edgeR} are obtained as follows:
...
<<maplot_of_estimated_result, fig = TRUE, size = 0.8, png = TRUE, pdf = FALSE, eps = FALSE>>=
plot(tcc)
@
...
<<idegerEdger_tcc_bayseq_summary>>=
set.seed(1000)
library(TCC)
data(hypoData)
samplesize <- 100
...
<<idegerEdger_tcc_bayseq_org_summary>>=
set.seed(1000)
library(TCC)
data(hypoData)
samplesize <- 100
...
method implemented in \Rpackage{edgeR} (i.e., the exact test; ver. 3.4) 
...
\Rpackage{edgeR} together with iDEGES/\Rpackage{edgeR} normalization 
