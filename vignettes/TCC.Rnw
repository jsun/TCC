...
model and reduced model). The full model (\Robject{full}) 
and reduced model (\Robject{reduced}) 
...
(\Robject{full} and \Robject{reduced}) can be performed as follows.
...
The two models (\Robject{full} and \Robject{reduced}) will automatically be
...
tcc <- new("TCC", hypoData, group)
tcc <- calcNormFactors(tcc, norm.method = "deseq2", iteration = 0)
tcc <- estimateDE(tcc, test.method = "deseq2", contrast = c("group", "1", "2"), FDR = 0.1)
result <- getResult(tcc, sort = TRUE)
head(result)
library(ROC)
truth <- c(rep(1, 200), rep(0, 800))
