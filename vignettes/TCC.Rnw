...
enabling advanced users familiar with the existing packages can easily learn
what is done in \Rpackage{TCC}. Note that end users, who just want to perform robust
...
Since GLMs approach, packaged in \Rpackage{edgeR}, requires the arguments
...
Let us explain the first approach for normalizing paired two-group count data.
