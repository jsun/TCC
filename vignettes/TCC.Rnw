...
(i.e., DEGES) implemented in TCC. Alternative DEGES procedures consisting
of functions in other packages (edgeR, DESeq, and baySeq) are also provided,
enabling advanced users familiar with the existing packages can easily learn
what is done in TCC. Note that end users, who just want to perform robust
differential expression analysis using TCC,
